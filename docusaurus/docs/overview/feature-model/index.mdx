...
Objective measurements of things in the world follow [The International System of Units (SI)](https://www.bipm.org/en/publications/si-brochure), meaning heights, widths, lengths, etc. that represent real-world properties of features. In the schema, these values are provided as scalar numeric value without units, since the units are implied and documented in place in the schema. Overture does this to maximize consistency and predictability. 

Quantities specified in regulatory rules, norms and customs follow local specifications wherever possible. In the schema, these values are provided as two-element arrays where the first element is the scalar numeric value and the second value is the units.
Overture uses local units to ensure that norms are specified in the most relevant way to the people who live there. Overture uses units to maximize consistency and predictability in the schema.
