...
Measurements in the schema follow [The International System of Units (SI)](https://www.bipm.org/en/publications/si-brochure), e.g. meters for building heights. Regulations and restrictions encoded in the schema follow local units. Speed limits, for example, appear as miles per hour (mph) in features located in the United States. Features in Germany show kilometers per hour (kph) for speed limits.


## Data formats

Overture describes data using a GeoJSON mental model and represents data as GeoJSON features. Instead of releasing data in that format, Overture made the decision to distribute data as [GeoParquet](https://geoparquet.org/). This documentation includes many examples of how to work with data stored in GeoParquet files.
