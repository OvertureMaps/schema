...
### Key Specifications
1. The Overture Data Schema is documented in the [JSON Schema](https://json-schema.org/), a widely used, machine-readable database documentation method.
2. We use GeoJSON as a mental model. The final format for Overture deliveries is still to be defined.
3. Property names use camelCase descrbed in the [Google JSON Style Guide](https://google.github.io/styleguide/jsoncstyleguide.xml)
4. Feature Geometries follow the [OGC Simple Feature Access specifications](https://www.ogc.org/standard/sfa/)
5. Measurements will use SI units (meters for building heights) and local units for regulations and restrictions (speed limits in mph in the US and km/h in Germany).
