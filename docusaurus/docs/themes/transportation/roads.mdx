...
street, or path, including a dedicated path for walking or cycling
(but excluding a railway). Road segments comprise the majority of
ground-based transportation segments. Roads are modeled using
...
Unlike many road segment properties, a road segment's `road.class`
property does not support
[geometric scoping](scoping-rules#geometric-scoping-linear-referencing)
(linear referencing). Consequently whenever a linear range of real-world
road makes a class transition (for example, between `secondary` and
`residential`), the Overture Transportation
[segmentation](shape-connectivity#segmentation) algorithm will generate
a segment split.
...
the <code>class</code> tag is heavily inspired by OpenStreetMap. In this
case, it is similar to, but not the same as,
OSM's <code>highway=*</code> tag.
</em>
</p>
...
`road.flags = [isLink, isUnderConstruction]` is a link segment, and is
...
[rules](scoping-rules#rules-and-rule-based-properties) will be used to
...
local rules, norms, and customs. An example of a turn restriction
...
along the `backward` heading. An example of a turn restriction
...
set of allowed transitions may be reduced by adding explicit turn
restrictions in the `road.restrictions.prohibitedTurns` property.
...
customs. (As with access and turn restrictions, the Overture
...
`road.restrictions.speedLimits`. (Granular speed limits can also be
...
specified using [rules](scoping-rules#rules-and-rule-based-properties).
...
[rules](scoping-rules) for resolving the lane block applicable at any
...
The number of lanes at a given place/time on a road segment is equal to
...
be chosen using the
[rule evaluation algorithm](scoping-rules#rule-evaluation-algorithm)
...
*A segment with two [geometrically-scoped](scoping-rules#geometric-scoping-linear-referencing)
