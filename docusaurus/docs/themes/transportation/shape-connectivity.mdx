...
| `bothWays` | Travel is allowed both `forward` and `backward`. |
...
`bothWays`) should be used along with Temporal Scoping. [**TODO: LINK**].
...
at greater length in the page on Scoped Properties. **TODO: Link here.**
