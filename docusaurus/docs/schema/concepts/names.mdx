...
Named entities in Overture Maps share a common schema.

<JSONSchemaViewer
  schema={ yamlLoad(MainDefs) }
  resolverOptions={ generateResolverOptions( {yamlBasePath: '/', jsonPointer:"#/$defs/propertyDefinitions/allNames" })}/>

## `primary`
If a feature has a name, expect the `names` attribute to have at least one property: `primary`. This will always be populated with the localized name of a feature.
...
Rules can also have `side` and `geometric` range scoping. Read more about [scoping rules here](/schema/attributes/scoping-rules).
