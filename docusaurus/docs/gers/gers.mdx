...
Right now there are two on-ramps for getting started with GERS. Both of them require you to reach out to us at data@overturemaps.org

1. Contribute you data to Overture, in which case we will generate new GERS IDs or associate existing IDs for your dataset as it relates to the current Overture corpus (of which your data is now a part).
2. Conflate your data against a current Overture dataset, identifying matches between existing map objects with a GERS ID and your own data.
...
We also invite interested parties to consider [joining the Overture Maps Foundation at one of our membership levels](https://overturemaps.org/become-a-member/).
