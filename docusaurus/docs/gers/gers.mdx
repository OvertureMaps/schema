...
Here is an example query that gives us a short list of GERS IDs from the buildings theme in Overture's January 2024 data release.
...
GERS IDs are stable (within a reasonable tolerance of error) across multiple versions of Overture data. Stability When stability is not possible, traceability will be provided. For example:
