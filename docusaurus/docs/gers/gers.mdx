...
The H3 index reflects the location of an entity when Overture first assigns it an ID. Because GERS IDs are intended to be stable over time, Overture will not generate a new ID for that entity if its location moves within its assigned H3 layer. Put another way, a GERS ID should be treated as a unique identifier and nothing more. Overture discourages users from attempting to extract information about an entity from its GERS ID.
