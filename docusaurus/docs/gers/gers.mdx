...
An `entity` is basically a thing in the real world. That thing could be a segment of road, a city, a grocery store, a building, a park bench or a lamp post. An `entity` in GERS is assigned a unique GERS ID. These IDs are useful to anyone looking to match their data with Overture data. 
...
## Components of a GERS IDs

GERS IDs have a consistent format: 

`data theme identifier + H3 tile address + 56-bit value generated uniformly across a given theme`

Here you can see a list of GERS IDs from Overture's recent release of buildings data, generated from a [duckdb](https://duckdb.org/) [query of the GeoParquet file](https://github.com/OvertureMaps/data). 
...
## Why Overture is building GERS

- We're building GERS to make it easier to share, sell, and integrate map data.
- Mapping data is too hard to work with. Companies spend more time and money integrating GIS datasets than they do licensing them.
- A common ID system enables easier creation, commerce, and cooperation.
