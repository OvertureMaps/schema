...
## Licensing
For Overture data layers which are CDLA Permissive 2.0, there are no restrictions on use of external data other than the attribution requirements.  As with many other Overture projects, GERS will be licensed CDLA v.2.
For Overture data layers which are ODbL, they must adhere to ODbL license terms, specifically whether any referenced data is considered a collective database or a derivative database. See OpenStreetMap Community Guidelines for definitions on the differences.
