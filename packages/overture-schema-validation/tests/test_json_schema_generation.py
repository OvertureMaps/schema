...
def create_field_constraint_model(field_type, constraint_instance):
    ...
    class TestModel(ConstraintValidatedModel, BaseModel):
        test_field: Annotated[field_type, constraint_instance]

    return TestModel


def assert_pattern_constraint(
    schema, field_name, expected_pattern, expected_description=None
):
...
def assert_range_constraint(
    schema, field_name, min_val=None, max_val=None, description=None
):
...
def assert_collection_constraint(
    schema, field_name, min_items=None, max_items=None, unique_items=None
):
...
class TestJSONSchemaGeneration:
    ...
    def test_exactly_one_of_constraint_json_schema(self):
    ...
    def test_pattern_constraint_json_schema(self):
    ...
    def test_language_tag_constraint_json_schema(self):
    ...
    def test_country_code_constraint_json_schema(self):
    ...
    def test_region_code_constraint_json_schema(self):
    ...
    def test_hex_color_constraint_json_schema(self):
    ...
    def test_no_whitespace_constraint_json_schema(self):
    ...
    def test_iso8601_datetime_constraint_json_schema(self):
    ...
    def test_json_pointer_constraint_json_schema(self):
    ...
    def test_whitespace_constraint_json_schema(self):
    ...
    def test_min_items_constraint_json_schema(self):
        """Test MinItemsConstraint JSON schema generation."""
        constraint = MinItemsConstraint(3)

        # Test with array type
        TestModel = create_field_constraint_model(list[str], constraint)
        schema = TestModel.model_json_schema()
        assert_collection_constraint(schema, "test_field", min_items=3)

        # Test with dict type (should use minProperties)
        TestModel = create_field_constraint_model(dict[str, str], constraint)
        schema = TestModel.model_json_schema()
        assert "properties" in schema
        assert "test_field" in schema["properties"]
        field_schema = schema["properties"]["test_field"]
        assert field_schema.get("minProperties") == 3

    def test_unique_items_constraint_json_schema(self):
    ...
    def test_composite_unique_constraint_json_schema(self):
        """Test CompositeUniqueConstraint JSON schema generation."""
        constraint = CompositeUniqueConstraint("name", "type")
        TestModel = create_field_constraint_model(list[dict], constraint)
        schema = TestModel.model_json_schema()

        assert "properties" in schema
        assert "test_field" in schema["properties"]
        field_schema = schema["properties"]["test_field"]
        assert field_schema.get("uniqueItems")
        assert "name, type" in field_schema.get("description", "")

    def test_confidence_score_constraint_json_schema(self):
    ...
    def test_zoom_level_constraint_json_schema(self):
        """Test ZoomLevelConstraint JSON schema generation."""
        constraint = ZoomLevelConstraint()
        TestModel = create_field_constraint_model(int, constraint)
        schema = TestModel.model_json_schema()
        ...
        assert_range_constraint(
            ...
            min_val=0,
            max_val=23,
            description="Map zoom level between 0 and 23",
        )

    def test_linear_reference_range_constraint_json_schema(self):
    ...
    def test_pattern_properties_dict_constraint_json_schema(self):
