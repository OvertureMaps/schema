...
"""Water validation expression builders."""
...
def _type_enum_check() -> Check:
    return Check(
        field="type",
        name="enum",
        expr=check_enum(F.col("type"), ["water"]),
        shape=CheckShape.SCALAR,
        read_columns=frozenset({"type"}),
    )
...
def water_checks() -> list[Check]:
    """All validation checks for water."""
    return [
        _id_required_check(),
...
WATER_SCHEMA = StructType(
    [
        ...
        StructField("is_intermittent", BooleanType(), True),
        StructField("is_salt", BooleanType(), True),
...
ENTRY_POINT = "overture.schema.base:Water"
