...
"""Division Area validation expression builders."""
...
def _type_enum_check() -> Check:
    return Check(
        field="type",
        name="enum",
        expr=check_enum(F.col("type"), ["division_area"]),
        shape=CheckShape.SCALAR,
        read_columns=frozenset({"type"}),
    )
...
def _division_id_required_check() -> Check:
    return Check(
        field="division_id",
        name="required",
        expr=check_required(F.col("division_id")),
        shape=CheckShape.SCALAR,
        read_columns=frozenset({"division_id"}),
    )


def _division_id_string_min_length_check() -> Check:
    return Check(
        field="division_id",
        name="string_min_length",
        expr=check_string_min_length(F.col("division_id"), 1),
        shape=CheckShape.SCALAR,
        read_columns=frozenset({"division_id"}),
    )


def _division_id_no_whitespace_check() -> Check:
    return Check(
        field="division_id",
        name="no_whitespace",
        expr=check_pattern(
...
def division_area_checks() -> list[Check]:
    """All validation checks for division_area."""
    return [
        _names_check(),
        ...
        _class_enum_check(),
        _division_id_required_check(),
        _division_id_string_min_length_check(),
        _division_id_no_whitespace_check(),
...
DIVISION_AREA_SCHEMA = StructType(
    [
        StructField(
            "names",
            StructType(
        ...
        StructField("is_land", BooleanType(), True),
        StructField("is_territorial", BooleanType(), True),
...
ENTRY_POINT = "overture.schema.divisions:DivisionArea"

PARTITIONS: dict[str, str] = {"theme": "divisions"}

MODEL_VALIDATION = ModelValidation(
    schema=DIVISION_AREA_SCHEMA,
    checks=division_area_checks,
