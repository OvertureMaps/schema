...
def test_select_models_by_extension_tag() -> None:
    models = discover_models(apply_extensions=False)
    # Wrappers surface only through the explicit introspection parameter;
    # the tag then narrows the listing to extension entries.
    extensions = select_models(
        models,
...
def test_wrapper_is_not_resolvable_by_name() -> None:
    # The standalone wrapper is internal merge machinery: naming it selects
    # nothing, so the CLI cannot validate a bare extension payload against it.
    models = discover_models(apply_extensions=False)
    assert select_models(models, type_names=("operating_hours",)) == {}


def test_select_models_applies_extensions_after_selection() -> None:
    models = discover_models(apply_extensions=False)
    selected = select_models(models, type_names=("place",))
    place = next(m for k, m in selected.items() if k.name == "place")
    assert "operating_hours" in _model(place).model_fields


def test_select_models_excluding_extensions_leaves_place_bare() -> None:
    # The exclusion must strip the merged field, not merely hide the
    # standalone wrapper entry.
    models = discover_models(apply_extensions=False)
