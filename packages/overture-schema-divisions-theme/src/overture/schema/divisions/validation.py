...
class ParentDivisionValidator(BaseConstraintValidator):
    ...
    def get_json_schema_metadata(
        ...
        # Convert enum values to their string representation for JSON schema
        exempt_value = self.exempt_value
        if hasattr(exempt_value, "value"):
            exempt_value = exempt_value.value

        return {
            "type": "parent_division_required_unless",
            "field_name": field_name,
            "exempt_value": exempt_value,
            "if": {"properties": {field_name: {"const": exempt_value}}},
