...
Features in the `Water` type within Overture's `base` theme come form OpenStreetMap. The **subtype** and **class** of an Overture feature is determined by the tags that are present in OpenStreetMap.
