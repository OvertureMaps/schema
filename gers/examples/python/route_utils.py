...
def get_route_step_dist(
    feat_before_from: MatchableFeature,
    feat_from: MatchableFeature,
    feat_to: MatchableFeature,
    start_feature: MatchableFeature,
    end_feature: MatchableFeature,
    start_point: Point,
    end_point: Point,
) -> tuple[Point, float]:
    ...
    if feat_from.id == start_feature.id:
        ...
        p0_before, feat_from_entry_point = nearest_points(
            feat_before_from.geometry, feat_from.geometry
        )
...
def get_shortest_route(
    features: Iterable[MatchableFeature],
    feature_id_to_connected_features: dict[str, Iterable[MatchableFeature]],
    start_feature: MatchableFeature,
    end_feature: MatchableFeature,
    start_point: Point,
    end_point: Point,
    ...
    while len(feats_to_visit) > 0:
        ...
        for v in connected_features:
            if (
                v.id not in allowed_ids
                or (v.id in blocked_ids)
                or v.id not in ids_to_visit
            ):
                continue
            ...
            via_point, d = get_route_step_dist(
                prev[current_feature.id],
                current_feature,
                v,
                start_feature,
                end_feature,
    ...
    if prev[current_feature.id] is not None or current_feature.id == start_feature.id:
        while current_feature is not None:
            steps.insert(
                0, RouteStep(current_feature, prev_via_point[current_feature.id])
            )
