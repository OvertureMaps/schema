...
class MatchableFeature:
    ...
    def __init__(
        self, id: str, geometry: BaseGeometry, properties: dict = None
    ) -> None:
        self.id = str(id)
    ...
    def get_connector_ids(self) -> Iterable[str]:
        return (
            self.properties["connector_ids"]
            if self.properties is not None and "connector_ids" in self.properties
            else []
        )
...
class MatchableFeaturesSet:
    ...
    def __init__(
        self,
        features: dict[str, Iterable[MatchableFeature]],
        cells_by_id: dict[str, Iterable[str]],
        features_by_cell: dict[str, Iterable[MatchableFeature]],
    ) -> None:
...
class MatchedFeature:
    ...
    def __init__(
        self,
        id: str,
        matched_feature: MatchableFeature,
        overlapping_geometry: BaseGeometry,
        score: float,
        source_lr: Iterable[float] = None,
        candidate_lr: Iterable[float] = None,
    ) -> None:
        """
        Attributes:
    ...
    def to_json(self):
        j = {
            ...
            "overlapping_wkt": self.overlapping_geometry.wkt
            if self.overlapping_geometry is not None
            else None,
            "score": self.score,
        }
...
class TraceSnapOptions:
    ...
    def __init__(
        ...
        broken_distance_gap_reset_sequence=constants.DEFAULT_BROKEN_DISTANCE_GAP_RESET_SEQUENCE,
    ) -> None:
        self.sigma = sigma
        self.beta = beta
        self.allow_loops = allow_loops
        self.max_point_to_road_distance = max_point_to_road_distance
        self.max_route_to_trace_distance_difference = (
...
class SnappedPointPrediction:
    ...
    def __init__(
        ...
        id: str,
        snapped_point: Point,
        referenced_feature: MatchableFeature,
        distance_to_snapped_road: float,
        route_distance_to_prev_point: float,
        emission_prob: float,
        best_transition_prob: float,
        best_log_prob: float,
        best_prev_prediction: float,
        best_sequence: Iterable[str],
        best_route_via_points: Iterable[str],
        best_revisited_via_points_count: int,
        best_revisited_segments_count: int,
    ) -> None:
        self.id = str(id)
...
class PointSnapInfo:
    ...
    def __init__(
        ...
        time: str,
        seconds_since_prev_point: float = None,
        predictions: Iterable[SnappedPointPrediction] = [],
    ) -> None:
        self.index = index
    ...
    def to_json(
        ...
        best_prediction_json = (
            ...
            else self.best_prediction.to_json(diagnostic_mode)
        )
        ...
        if include_all_predictions:
            j["predictions"] = list(
                map(lambda x: x.to_json(diagnostic_mode), self.predictions)
            )
        return j
...
class TraceMatchResult:
    ...
    def __init__(
        ...
        id: str,
        source_wkt: str,
        points: Iterable[PointSnapInfo],
        source_length: float,
        target_candidates_count: int,
        matched_target_ids: Iterable[str] = None,
        elapsed: float = None,
        sequence_breaks: int = 0,
        points_with_matches: int = 0,
        route_length: float = 0,
        avg_dist_to_road: float = None,
        revisited_via_points: int = 0,
        revisited_segments: int = 0,
    ) -> None:
        self.id = id
    ...
    def to_json(self, diagnostic_mode=False, include_all_predictions=False):
        points_json = list(
            map(
                lambda x: x.to_json(diagnostic_mode, include_all_predictions),
                self.points,
            )
        )
        return {
