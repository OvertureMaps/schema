...
class TestTraces(unittest.TestCase):
    def test_match_traces(self):
        ...
        overture_file = os.path.join(
            constants.DATA_DIR, "overture-transportation-macon.geojson"
        )
        res = 12

        to_match = load_matchable_set(
            features_to_match_file, is_multiline=False, res=res
        )
        ...
        overture = load_matchable_set(
            overture_file,
            is_multiline=True,
            properties_filter={"type": "segment"},
            res=res,
        )
