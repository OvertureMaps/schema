...
def matches_properties_filter(feature: dict[str, Any], properties_filter: dict[str, Any]) -> bool:
    ...
    for prop in properties_filter:
        ...
        if prop not in feat_props or (properties_filter[prop] != "*" and feat_props[prop] != properties_filter[prop]):
            return False
    return True
        ...
        for row in reader:
            ...
            for k, v in row.items():
                ...
                if "id" not in feat_dict and "id" in key:
                    feat_dict["id"] = v
                elif "geometry" not in feat_dict and ("geometry" in key or "wkt" in key):
