...
Connectors align with places where segments meet with each other so either on segment ends or in intermediate points along segments.
Coordinate of a connector always intersects with segment's geometry (note: it doesn't mean that coordinate of connector must be
